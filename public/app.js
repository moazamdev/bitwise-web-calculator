...
// Decimal To Binary
const decToBin = (decValueInString) => {
	if (isDecimal(decValueInString)) {
		return;
	}
	let decValue = parseInt(decValueInString);
...
let isHexa = (hexValueInString) => {
	hexValueInString.split("").forEach((char) => {
		if ((char >= "G" && char <= "Z") || (char >= "g" && char <= "z")) {
			inputField.value = "";
			showErrorMsg();
			return true;
		}
